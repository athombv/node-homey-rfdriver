...
module.exports = class Scripts {
	...
	static checkTemplates(config, rootDir) {
		config.drivers.forEach(driver => {
			...
			driver.pair.forEach((view, index) => {
				...
				if (template) {
					template.id = template.id || templateName[3];
	...
	static setTemplateOptions(config, template, view, index, driver) {
		if (template.options.navigation) {
		...
		if (template.locales) {
			...
			const traverse = (obj, appLocale, path = []) => {
				for (const key in obj) {
					if (typeof obj[key] === 'function') {
						obj[key] = obj[key](driver, view, config);
					}
			...
			traverse(template.locales, config.locales);
		}
	}
