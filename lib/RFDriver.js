...
module.exports = class RFDriver extends Homey.Driver {
  ...
  async onPair(session) {
    session.registerHandler('showView', async viewId => {
      ...
      if (viewId === 'rf_transmitter_learn') return this.onPairRFTransmitter(session);
      if (viewId === 'rf_ir_remote_add') return this.onPairIRRemote(session);
    });
  }
  ...
  async onPairIRRemote(session) {
    await session.emit('createDevice', {
      name: this[sRF].name,
      data: {
        id: RFUtil.generateUuidV4(),
      }
    });
  }
